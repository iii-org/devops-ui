...
export default {
  ...
  methods: {
    fetchData() {
      Promise.all([getIssuePriority(), getIssueTracker(), getIssueStatus()]).then(res => {
        this.issuePriorityList = res[0].data.map(item => {
          return { label: item.name, value: item.id }
        })
        this.issueTypeList = res[1].data.map(item => {
          return { label: item.name, value: item.id }
        })
        this.issueStatusList = res[2].data.map(item => {
          return { label: item.name, value: item.id }
        })
      })
    },
