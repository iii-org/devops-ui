      ...
      <el-table :data="pagedData" fit>
        ...
        <el-table-column align="center" :label="$t('general.Name')">
          <template slot-scope="scope">
            <el-link type="primary" :underline="false" @click="handleParticipateDialog(scope.row.id, scope.row.name)">
              {{ scope.row.name }}
            </el-link>
          </template>
        </el-table-column>
...
export default {
  ...
  computed: {
    ...mapGetters(['userRole', 'userId', 'selectedProjectId']),
    disabledEditOwner() {
