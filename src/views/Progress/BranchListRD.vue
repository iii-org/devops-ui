<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column-time label="Commit Time" :show-overflow-tooltip="true" prop="last_commit_time" />
      <el-table-column :label="$t('Git.Commit')" :show-overflow-tooltip="true" min-width="100" align="center">
        <template slot-scope="scope">
          <el-link :href="scope.row.commit_url" target="_blank" type="primary">
            {{ scope.row.short_id }}
          </el-link>
        </template>
      </el-table-column>
    ...
    <el-dialog title="Delete Branch" :visible.sync="deleteDialogVisible" width="50%">
      <h3><i class="el-icon-warning" style="color:#F56C6C" /> Please Input Full Branch Name to confirm the deletion</h3>
      <el-input v-model="deleteBranchName" placeholder="" />
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...mapActions(['branches/getBranchesByProject', 'branches/newBranch', 'branches/deleteBranch', 'tags/newTag']),
    async fetchData() {
    ...
    async handleDeleteModal() {
      if (!this.$route.params.bId) return
      if (this.selectedBranch !== this.deleteBranchName) {
        return this.$message({
