...
/* exported ics */
/* source: https://github.com/nwcell/ics.js/ */
  ...
  return {
    ...
    'addEvent': function(subject, description, location, begin, stop, rrule) {
      ...
      if (rrule) {
        if (!rrule.rrule) {
          if (rrule.freq !== 'YEARLY' && rrule.freq !== 'MONTHLY' && rrule.freq !== 'WEEKLY' && rrule.freq !== 'DAILY') {
            throw new "Recurrence rrule frequency must be provided and be one of the following: 'YEARLY', 'MONTHLY', 'WEEKLY', or 'DAILY'"()
          }

          if (rrule.until) {
            if (isNaN(Date.parse(rrule.until))) {
              throw new "Recurrence rrule 'until' must be a valid date string"()
            }
          }

          if (rrule.interval) {
            if (isNaN(parseInt(rrule.interval))) {
              throw new "Recurrence rrule 'interval' must be an integer"()
            }
          }

          if (rrule.count) {
            if (isNaN(parseInt(rrule.count))) {
              throw new "Recurrence rrule 'count' must be an integer"()
            }
          }

          if (typeof rrule.byday !== 'undefined') {
            if ((Object.prototype.toString.call(rrule.byday) !== '[object Array]')) {
              throw new "Recurrence rrule 'byday' must be an array"()
            }

            if (rrule.byday.length > 7) {
              throw new "Recurrence rrule 'byday' array must not be longer than the 7 days in a week"()
            }
            ...
            for (const d in rrule.byday) {
              if (BYDAY_VALUES.indexOf(rrule.byday[d]) < 0) {
                throw new "Recurrence rrule 'byday' values must include only the following: 'SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'"()
              }
            }
          }
        }
      ...
      let calendarEvent = [
        ...
        'CLASS:PUBLIC',
        'DESCRIPTION:' + (description.match(`href=['"](?<url>.*?)['"]`).groups.url || description),
